...
public abstract class AbstractCompositeTree extends BaseBTree implements CompositeBTree {
    ...
    protected AbstractCompositeTree(SelectorEnum selectorEnum,boolean type,Action action){
        super(selectorEnum,type);
        this.action = action;
    }
    ...
    public EStatus handleRequest(Map<String, Object> request) {
        EStatus handle = handleWayMap.get(selectorEnum).handle(children, request);
        if (action != null){
            action.action(this);
        }
        return handle;
