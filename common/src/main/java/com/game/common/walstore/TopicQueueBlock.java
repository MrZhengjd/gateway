...
import com.game.common.store.PageIndex;
...
public class TopicQueueBlock {
    ...
    public TopicQueueBlock(String blockFilePath, PageIndex index, RandomAccessFile blockFile, FileChannel fileChannel,
...
//        DataUtils.printStackTrace();
    ...
    public TopicQueueBlock(PageIndex index, String blockFilePath) {
        this.index = index;
        this.blockFilePath = blockFilePath;
...
//        DataUtils.printStackTrace();
    ...
    public void putEOF() {
        this.byteBuffer.position(index.getWriterPosition());
        this.byteBuffer.putInt(EOF);
    }

    public boolean isSpaceAvailable(int len) {
        int increment = len + 4;
        int writePosition = index.getWriterPosition();
    ...
    public int write(byte[] bytes) {
        ...
        index.setWriterPosition(increment + writePosition);
//        index.(index.getWriteCounter() + 1);
        return increment;
    }
...
//        int writeNum = index.getWriteNum();
        ...
        index.setReadPosition(readPosition + bytes.length + 4);
