    ...
    public void init(){
        synchronized (DATA_BACKUP_PATH.intern()){
            if (!inital ){
                ...
                UnLockWALQueue queue = new UnLockWALQueue(MasterInfo.getInfo()+"queue", configPath0+"master", 0, eventExecutor);
                this.queue = queue;
                UnLockWALQueue indexQueue = new UnLockWALQueue(MasterInfo.getInfo()+"index", configPath0+"index", 0, index);
                this.masterChangeQueue = new UnLockWALQueue(MasterInfo.getInfo()+"master", configPath0+"master", 0, master);
